...
def build_plan(stage, plan_size="full"):
    ...
    if stage in ("sp04", "all"):
        bases = [(512, 256), (1024, 512)] if plan_size == "core" else [
            (512, 256),
            (768, 384),
            (1024, 512),
        ]
        for d_model, topk in bases:
            for ticks in [1, 2, 4]:
                add_sparse_experiment(
                    plan,
...
def is_sparsity_experiment(name):
...
def recommend_batches(args):
    ...
    for row in rows:
        ...
        if not base or not is_sparsity_experiment(base):
            continue
...
def parse_args():
    parser = argparse.ArgumentParser(description="CTM sparsity experiment plan helper")
    sub = parser.add_subparsers(dest="cmd", required=True)

    common = argparse.ArgumentParser(add_help=False)
    common.add_argument("--stage", default="all", choices=SPARSITY_STAGES)
    ...
    p.add_argument("--output", default="runs/experiment_plans/sparsity_plan.csv")
    p.add_argument("--batch_profile", default=None)
    p.set_defaults(func=print_commands)

    p = sub.add_parser("final-plan", parents=[common])
    p.add_argument("--output", default="runs/experiment_plans/sparsity_final_plan.csv")
    p.add_argument("--batch_profile", default="runs/metrics/sparsity_batch_profile.csv")
    ...
    p.add_argument("--output", default="runs/experiment_plans/sparsity_batch_tune_plan.csv")
    ...
    p.add_argument("--output", default="runs/metrics/sparsity_batch_profile_quick.csv")
    p.add_argument("--report_output", default="runs/metrics/sparsity_quick_probe_report.csv")
    ...
    p.add_argument("--metrics_dir", default="runs/metrics")
    p.add_argument("--output", default="runs/metrics/sparsity_summary.csv")
    p.set_defaults(func=summarize)

    p = sub.add_parser("recommend-batches")
    p.add_argument("--metrics_dir", default="runs/metrics")
    p.add_argument("--output", default="runs/metrics/sparsity_batch_profile.csv")
    p.add_argument("--target_memory_gb", type=float, default=80.0)
    p.add_argument("--memory_util", type=float, default=0.90)
    p.set_defaults(func=recommend_batches)

    p = sub.add_parser("batch-report")
    p.add_argument("--stage", default="all", choices=SPARSITY_STAGES)
    p.add_argument("--plan_size", default="full", choices=PLAN_SIZES)
    p.add_argument("--metrics_dir", default="runs/metrics")
    p.add_argument("--output", default="runs/metrics/sparsity_batch_probe_report.csv")
    p.add_argument("--batch_sizes", type=int, nargs="+", default=[2, 4, 6, 8, 10, 12])
    p.add_argument("--tune_steps", type=int, default=80)
    p.add_argument("--tune_log_interval", type=int, default=20)
