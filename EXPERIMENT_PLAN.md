...
This plan is designed for sequential pool submissions. Each experiment gets a
stable `experiment_name`, `swanlab_name`, `save_weight`, and metrics CSV under
`runs/metrics/`.
...
Then generate the full plan with per-experiment batch sizes:
...
Optional serial runner for the full plan:
...
- `iterations = 1, 2, 4, 8, 16`: raw tick scaling.
...
Includes `self_cond`, `cross_layer_state`, shallow synapse, short memory, and
tiny NLM ablations.
